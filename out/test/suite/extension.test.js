...
suite('MinifyAll Test Suite', () => {
    ...
    test('Extension can be activated', () => vscode.extensions.getExtension('josee9988.minifyall').activate().then(() => {
        assert.ok(true);
    }));
    ...
    test('Function \'getNewFilePath\' works', () => {
        const result = getNewFilePath_1.default(path, '/myFile.css', 'css');
        assert.deepStrictEqual(result, '/myFile-min.css');
    });
    test('Function \'removeComments\' works', () => {
        const result = globalMinifiers.removeComments(['const assert; // simple comment.', '// full line', 'console.log(1+1); /* multiLine simple*/', 'let variable; /*comment', 'just a comment', 'stillcomment*/', 'console.log("all done");']).join('');
