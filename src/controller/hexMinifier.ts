...
export default class HexMinifier {
  ...
  public shortHexMain(): void {
    for (let i = 0; i < this.cssContent.length; i++) {
      const hexadecimal: string[] = this.cssContent[i].match(/#[0-9a-fA-F]+/ig);
  ...
  public shortRGBMain(): void {
    for (let i = 0; i < this.cssContent.length; i++) {
      const rgb: string[] = this.cssContent[i].match(/((rgb)\((\d{1,3}%?,\s?){3}(1|0?\.\d+)\)|(rgb)\(\d{1,3}%?(,\s?\d{1,3}%?){2}\))/ig);
      if (rgb !== null) {
  ...
  public shortRGBAMain(): void {
    for (let i = 0; i < this.cssContent.length; i++) {
      const rgb: string[] = this.cssContent[i].match(/((rgba)\((\d{1,3}%?,\s?){3}(1|0?\.\d+)\)|(rgb)\(\d{1,3}%?(,\s?\d{1,3}%?){2}\))/ig);
      if (rgb !== null) {
        const rgbaString: string = rgb.toString();
        const percent: string[] = rgbaString.match(/[%]/g);
        if (percent === null) {
  ...
  public rgbArrayToObject(rgbString: string): any {
    ...
    const match: string[] = matchColors.exec(rgbString);
    return match ? {
